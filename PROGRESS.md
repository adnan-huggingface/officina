...
## Measuring is one command, and the repository has a map (2026-09-14)

The handoff's last three wishes. **`cargo xtask measure <script>`** takes the
script `cargo xtask author` reads — type, key, table, style, page-break —
authors the document through Scriva's own commands under `target/measure/`,
has Word render it through the service, and prints the comparison: "what
does Word do here" was a probe written by hand on the laptop, and is now one
line. **`MAP.md`** is generated by `cargo xtask map` and again by every
`cargo xtask check`, the way formatting is: every crate with the targets its
tests run under, every source file with its module's first sentence, every
menu row of both applications with the keys that reach it — walked by the
menu tests through the driver, so the table is what the keyboard does — and
the registry of measured facts the handoff asked for, gathered rather than
moved: the 94 comments that say an application was measured and name which,
