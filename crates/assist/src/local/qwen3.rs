//! Qwen3 with quantized weights, as `candle-transformers` 0.9.2 has it
//! (`models/quantized_qwen3.rs`, MIT OR Apache-2.0, © the candle authors),
//! brought in for one thing the original does not offer: **keeping part of
//! the keys and values between requests.**
//!
//! A request here is about a thousand tokens, seven hundred of which — the
//! brief and the tools — are the same on every request of a session, and
//! reading them is arithmetic over the whole prefix. The original can only
//! clear its cache whole. [`ModelWeights::truncate_kv_cache`] cuts it back
//! to a length instead, so that a request that begins as the last one did
//! reads only what follows; [`ModelWeights::kv_len`] says how much is held.
//! The `tracing` spans are gone, since this crate does not trace; nothing
//! else is changed, so that a newer candle can replace this file.
...
impl AttentionWeights {
    ...
    /// Keeps the keys and values of the first `len` positions and lets the
    /// rest go. Zero clears.
    fn truncate_kv_cache(&mut self, len: usize) -> Result<()> {
        if len == 0 {
            self.kv_cache.reset();
            return Ok(());
        }
        match self.kv_cache.current_seq_len() {
            held if held == len => return Ok(()),
            held if held < len => candle_core::bail!("the cache holds {held} positions, not {len}"),
            _ => {}
        }
        let dim = self.kv_cache.dim();
        if let Some(k) = self.kv_cache.k_mut() {
            *k = k.narrow(dim, 0, len)?.contiguous()?;
        }
        if let Some(v) = self.kv_cache.v_mut() {
            *v = v.narrow(dim, 0, len)?.contiguous()?;
        }
        Ok(())
    }
...
impl ModelWeights {
    ...
    /// Keeps the keys and values of the first `len` positions in every layer
    /// and lets the rest go, so that a prompt beginning with those positions
    /// can be read from `len` on. Zero clears.
    ...
    pub fn kv_len(&self) -> usize {
        self.layers
            .first()
            .map(|layer| layer.self_attn.kv_len())
            .unwrap_or(0)
    }
}
